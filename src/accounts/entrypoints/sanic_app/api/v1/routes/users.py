...
__all__ = ["get_users_bp"]


# Пользователь
def get_me_bp():
    users_me_bp = Blueprint("me", url_prefix="/me")
    users_me_bp.on_request(protected)
    users_me_bp.on_request(is_user_or_admin)
...
def get_users_bp():
    users_me_bp = get_me_bp()
    users_admin_bp = get_users_admin_bp()

    users_bp = Blueprint.group(users_me_bp, users_admin_bp, url_prefix="/users")
